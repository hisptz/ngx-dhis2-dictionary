...
export class IndicatorsEffects {
  ...
  ProgramIndicatorGroups$: Observable<any> = this.actions$.pipe(
    ofType<indicators.IndicatorsAction>(
      indicators.IndicatorsActions.LoadProgramIndicatorGroups
    ),
    switchMap(() =>
      this.httpClient
        .get(
        ...
        .pipe(
          map(
            (programIndicatorGroupsObject: any) =>
              new indicators.LoadProgramIndicatorGroupsSuccessAction(
                programIndicatorGroupsObject
              )
          ),
          catchError(error =>
            of(new indicators.LoadProgramIndicatorGroupsFailAction(error))
          )
        )
    )
  );
